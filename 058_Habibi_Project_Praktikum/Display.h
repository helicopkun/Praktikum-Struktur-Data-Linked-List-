...
void tampil_detail(Barang* b) {
    ...
    while (cur) {
        ...
        << " Hari"
        << "\n Rute (dari inventory): ";
        int target_lokasi = lokasi_inv;
        while (target_lokasi != -1) {
            cout << nama_semua_lokasi[target_lokasi];
            target_lokasi = cur->data->lokasi_gudang->lokasi_sebelum[target_lokasi];
            if (target_lokasi != -1) cout << " -> ";
        }
        cout << "\n\n";
        cur = cur->next;
    }
...
void tampil_gudang(Gudang* list[], int size, Jalur* lokasi_awal, Barang* b = nullptr) {
    for (int i = 0; i < size; i++) {
        ...
        cout << " Hari, Rute: ";
        int target_lokasi = lokasi_to_idx(lokasi_awal->lokasi_awal);
        while (target_lokasi != -1) {
            cout << nama_semua_lokasi[target_lokasi];
            target_lokasi = list[i]->lokasi_gudang->lokasi_sebelum[target_lokasi];
            if (target_lokasi != -1) cout << " -> ";
        }
        cout << "\n\n";
    }
...
void sort_closest_gudang(Gudang* list[], int size, Jalur* lokasi_awal) {
    Gudang* tempB;
    for (int i = 0; i < size; i++) {
        int min = i;
        for (int j = i; j < size; j++) {
            if (lokasi_awal->waktu_ke[lokasi_to_idx(list[j]->lokasi_gudang->lokasi_awal)] <
                lokasi_awal->waktu_ke[lokasi_to_idx(list[min]->lokasi_gudang->lokasi_awal)])
                min = j;
        }
        tempB = list[i];
        list[i] = list[min];
...
void menu_inventory_management() {
    ...
    while (true) {
        ...
        if (pilihan == 0) break;
            ...
            tampil_gudang(sublist, size, inventory.lokasi_inv, list_barang[subI]);
            ...
            inventory.update_barang_global(list_barang[subI], -amount);
...
void menu_gudang_management() {
    ...
    while (true) {
        ...
        cout << "\n\n";
        int pilihan = input_idx("  Gudang:\n[0]Exit, [1]Transfer item, [2]Buy item, [3]Add item, [4]Register Gudang\nPilihan: ", 0, 4);
        int gudang_idx;
        cout << "\n\n";
        if (pilihan == 0) break;
        ...
        if (pilihan == 1) {
            ...
            system("cls");
            tampil_gudang(listG, sizeT, list_gudang[gudang_idx]->lokasi_gudang, barang_list[item]);
            ...
            inventory.push_history(listG[gudang]->nama+" Mengirim "+to_string(amount)+" "+barang_list[item]->nama+" ke "+list_gudang[gudang_idx]->nama);
            ...
            inventory.push_history(list_gudang[gudang_idx]->nama+" Membeli "+to_string(amount)+" "+barang_list[item]->nama);
            save_gudang();
            save_inventory();
            save_riwayat();
            press_enter();
            ...
            inventory.push_history(list_gudang[gudang_idx]->nama+" Registered "+listB[add]->nama);
            save_gudang();
            save_riwayat();
            press_enter();
        }
